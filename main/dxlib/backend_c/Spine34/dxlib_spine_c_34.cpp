...
#include "dxlib_spine_c_34.h"


void CDxLibSpineDrawableC34::ensureWorldVertices(int count)
{
...
void CDxLibSpineDrawableC34::ensureDxLibVertices(int count)
{
...
void CDxLibSpineDrawableC34::ensureDxLibIndices(int count)
{
...
CDxLibSpineDrawableC34::CDxLibSpineDrawableC34(spSkeletonData* pSkeletonData, spAnimationStateData* pAnimationStateData)
{
	spBone_setYDown(-1);
...
CDxLibSpineDrawableC34::~CDxLibSpineDrawableC34()
{
...
spSkeleton* CDxLibSpineDrawableC34::skeleton() const noexcept { return m_skeleton; }
spAnimationState* CDxLibSpineDrawableC34::animationState() const noexcept { return m_animationState; }

void CDxLibSpineDrawableC34::premultiplyAlpha(bool premultiplied) noexcept { m_isAlphaPremultiplied = premultiplied; }
bool CDxLibSpineDrawableC34::isAlphaPremultiplied() const noexcept { return m_isAlphaPremultiplied; }
void CDxLibSpineDrawableC34::forceBlendModeNormal(bool toForce) noexcept { m_isToForceBlendModeNormal = toForce; }
bool CDxLibSpineDrawableC34::isBlendModeNormalForced() const noexcept { return m_isToForceBlendModeNormal; }

void CDxLibSpineDrawableC34::update(float fDelta)
{
...
void CDxLibSpineDrawableC34::draw()
{
	if (m_skeleton == nullptr || m_animationState == nullptr) return;
	...
	for (int i = 0; i < m_skeleton->slotsCount; ++i)
	{
		...
		int iDxLibBlendMode;
		spBlendMode blendMode = m_isToForceBlendModeNormal ? SP_BLEND_MODE_NORMAL : pSlot->data->blendMode;
		switch (blendMode)
		{
...
void CDxLibSpineDrawableC34::setLeaveOutList(const char** list, int listCount)
{
	clearLeaveOutList();
...
DxLib::FLOAT4 CDxLibSpineDrawableC34::getBoundingBox() const
{
	...
	for (int i = 0; i < m_skeleton->slotsCount; ++i)
	{
		...
		else if (pAtt->type == SP_ATTACHMENT_MESH)
		{
			...
			cnt = pMesh->super.worldVerticesLength;
			if (cnt > tempCap) { tempCap = cnt; pTemp = static_cast<float*>(realloc(pTemp, sizeof(float) * tempCap)); }
...
DxLib::FLOAT4 CDxLibSpineDrawableC34::getBoundingBoxOfSlot(const char* slotName, size_t nameLength, bool* found) const
{
	float fMinX = FLT_MAX, fMinY = FLT_MAX;
	float fMaxX = -FLT_MAX, fMaxY = -FLT_MAX;
...
void CDxLibSpineDrawableC34::clearLeaveOutList()
{
...
bool CDxLibSpineDrawableC34::isSlotToBeLeftOut(const char* slotName)
{
...
bool CDxLibSpineDrawableC34::getSlotMeshData(const char* slotName, size_t nameLength, SlotMeshData& outData) const
{
