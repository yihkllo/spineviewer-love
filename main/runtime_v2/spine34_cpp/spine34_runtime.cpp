...
struct RegionAttachment
{
    ...
    bool linkedMesh = false;
    bool inheritDeform = true;
    std::string parentSkinName;
...
public:
    ...
    std::size_t Offset() const noexcept { return static_cast<std::size_t>(m_pos - m_begin); }
    ...
    std::string String()
    {
        int byteCountWithTerminator = VarInt(true);
        if (byteCountWithTerminator == 0)
            return std::string();
        if (byteCountWithTerminator == 1)
            return std::string();

        const int byteCount = byteCountWithTerminator - 1;
    ...
    std::vector<float> FloatValues(int count, float scale)
    {
        std::vector<float> values;
        if (count <= 0)
            return values;
...
class Spine34CppRuntime final : public IRuntime
{
public:
    RuntimeInfo Info() const noexcept override
    {
        return RuntimeInfo{ RuntimeKind::Cpp34, "3.4", "3.4 C++" };
    }
...
private:
    bool LoadAtlasText(const LoadRequest& request, std::string& outText, std::string& outBasePath)
    {
        ...
        {
            m_lastError = "Spine 3.4 load failed: atlas file is missing.";
            return false;
        }
        ...
        {
            m_lastError = "Spine 3.4 load failed: could not read atlas file: " + atlasPath;
            return false;
        }
                           ...
                           const std::vector<std::string>& textureDirectories)
    {
        ...
        if (pages.empty())
        {
            m_lastError = "Spine 3.4 load failed: atlas has no texture pages.";
            return false;
        }
    ...
    bool LoadSkeletonJsonText(const LoadRequest& request, std::string& outText)
    {
        ...
        {
            m_lastError = "Spine 3.4 load failed: skeleton json is missing.";
            return false;
        }

        if (!ReadTextFile(request.skeletonPaths.front(), outText))
        {
            m_lastError = "Spine 3.4 load failed: could not read skeleton json: " + request.skeletonPaths.front();
            return false;
        }
        return true;
    }
    ...
    bool LoadSkeletonBinaryData(const LoadRequest& request, std::string& outData)
    {
        ...
        if (request.skeletonPaths.empty())
        {
            m_lastError = "Spine 3.4 binary load failed: skeleton data is missing.";
            return false;
        }

        if (!ReadTextFile(request.skeletonPaths.front(), outData))
        {
            m_lastError = "Spine 3.4 binary load failed: could not read skeleton file: " + request.skeletonPaths.front();
            return false;
        }
        return true;
    }
    ...
    bool LoadJsonMetadata(const std::string& jsonText)
    {
        ...
        {
            m_lastError = "Spine 3.4 load failed: json contains no slots.";
            return false;
        }
    ...
    void ApplyAnimationTimelines(const AnimationData& animation, float time, std::vector<BonePose>& bones, std::vector<SlotPose>& slots) const
    {
        for (const auto& entry : animation.bones)
        {
            ...
            if (!timeline.shear.empty() && time >= timeline.shear.front().time)
            {
                const FloatFrame frame = SampleFloatFrame(timeline.shear, time);
                pose.shearX = m_bones[static_cast<std::size_t>(boneIndex)].shearX + frame.a;
                pose.shearY = m_bones[static_cast<std::size_t>(boneIndex)].shearY + frame.b;
            }
        }
    ...
    void BuildBoneWorld(const std::vector<BonePose>& bonePose, std::vector<Mat2D>& boneWorld) const
    {
        ...
        for (std::size_t i = 0; i < m_bones.size(); ++i)
        {
            ...
            const auto makeBasis = [](float rotation, float shearX, float shearY, float scaleX, float scaleY)
            {
                Mat2D basis{};
                const float xRadians = DegToRad(rotation + shearX);
                const float yRadians = DegToRad(rotation + 90.0f + shearY);
                basis.a = std::cos(xRadians) * scaleX;
                basis.b = std::sin(xRadians) * scaleX;
                basis.c = std::cos(yRadians) * scaleY;
                basis.d = std::sin(yRadians) * scaleY;
                basis.worldScaleX = scaleX;
                basis.worldScaleY = scaleY;
                basis.worldRotation = rotation;
                return basis;
            };
            ...
            if (bone.parent >= 0 && static_cast<std::size_t>(bone.parent) < boneWorld.size())
            {
                ...
                Mat2D local = makeBasis(pose.rotation, pose.shearX, pose.shearY, pose.scaleX, pose.scaleY);
                if (bone.inheritRotation && bone.inheritScale)
                {
                    Mat2D combined = Multiply(parent, local);
                    combined.tx = world.tx;
                    combined.ty = world.ty;
                    combined.worldScaleX = world.worldScaleX;
                    combined.worldScaleY = world.worldScaleY;
                    combined.worldRotation = world.worldRotation;
                    boneWorld[i] = combined;
                    continue;
                }

                const float basisRotation = bone.inheritRotation ? parent.worldRotation + pose.rotation : pose.rotation;
                const float basisScaleX = bone.inheritScale ? parent.worldScaleX * pose.scaleX : pose.scaleX;
                const float basisScaleY = bone.inheritScale ? parent.worldScaleY * pose.scaleY : pose.scaleY;
                Mat2D independent = makeBasis(basisRotation, pose.shearX, pose.shearY, basisScaleX, basisScaleY);
                independent.tx = world.tx;
                independent.ty = world.ty;
                independent.worldScaleX = world.worldScaleX;
                independent.worldScaleY = world.worldScaleY;
                independent.worldRotation = world.worldRotation;
                boneWorld[i] = independent;
                continue;
            }
            else
            {
            ...
            Mat2D basis = makeBasis(pose.rotation, pose.shearX, pose.shearY, pose.scaleX, pose.scaleY);
            basis.tx = world.tx;
            basis.ty = world.ty;
            basis.worldScaleX = world.worldScaleX;
            basis.worldScaleY = world.worldScaleY;
            basis.worldRotation = world.worldRotation;
            world = basis;
            boneWorld[i] = world;
        }
    }
    ...
    void LoadBones(const std::string& jsonText)
    {
        ...
        ForEachObjectInArray(jsonText, bonesBegin, bonesEnd, [this, &jsonText](std::size_t begin, std::size_t end)
        {
            ...
            ReadFloatPropertyInObject(jsonText, begin, end, "shearX", bone.shearX);
            ReadFloatPropertyInObject(jsonText, begin, end, "shearY", bone.shearY);
    ...
    bool LoadBinarySkeleton(const LoadRequest& request)
    {
        ...
        const auto setStage = [&reader](const std::string& stage)
        {
            reader.SetContext(stage);
        };
        const auto failIfBad = [this, &reader]() -> bool
        {
            if (reader.Ok())
                return false;
            std::ostringstream oss;
            oss << "Spine 3.4 binary load failed near " << reader.Context()
                << " at byte " << reader.Offset() << " / " << reader.Size() << ".";
            m_lastError = oss.str();
            return true;
        };

        setStage("header.hash");
        reader.String();
        if (failIfBad()) return false;
        setStage("header.version");
        reader.String();
        if (failIfBad()) return false;
        setStage("header.size");
        reader.Float();
        reader.Float();
        if (failIfBad()) return false;

        setStage("header.nonessential");
        const bool nonessential = reader.Bool();
        if (nonessential)
        {
            setStage("header.imagesPath");
            reader.String();
            if (failIfBad()) return false;
        }

        setStage("bones.count");
        const int boneCount = reader.VarInt(true);
        if (failIfBad()) return false;
        if (boneCount <= 0)
        {
            m_lastError = "Spine 3.4 binary load failed: no bones.";
            return false;
        }
        if (boneCount > 100000)
        {
            m_lastError = "Spine 3.4 binary load failed: invalid bone count.";
            return false;
        }
        ...
        {
            setStage("bones[" + std::to_string(i) + "]");
            BoneData bone{};
            bone.name = reader.String();
            const int parentIndex = i == 0 ? -1 : reader.VarInt(true);
            ...
            bone.shearX = reader.Float();
            bone.shearY = reader.Float();
        ...
        for (int i = 0; i < slotCount; ++i)
        {
            ...
            const int blend = reader.VarInt(true);
            if (blend == 1)
                slot.blend = BlendMode::Additive;
            else if (blend == 2)
                slot.blend = BlendMode::Multiply;
        ...
        if (failIfBad()) return false;
        if (ikCount < 0 || ikCount > 100000)
        {
            m_lastError = "Spine 3.4 binary load failed: invalid IK constraint count.";
            return false;
        }
        for (int i = 0; i < ikCount; ++i)
        {
            setStage("ik[" + std::to_string(i) + "]");
            IkConstraintData ik{};
            ik.name = reader.String();
            const int ikBoneCount = reader.VarInt(true);
            if (ikBoneCount < 0 || ikBoneCount > 100000)
            {
                m_lastError = "Spine 3.4 binary load failed: invalid IK bone count.";
                return false;
            }
        ...
        setStage("transformConstraints.count");
        const int transformConstraintCount = reader.VarInt(true);
        if (failIfBad()) return false;
        if (transformConstraintCount < 0 || transformConstraintCount > 100000)
        {
            m_lastError = "Spine 3.4 binary load failed: invalid transform constraint count.";
            return false;
        }
        for (int i = 0; i < transformConstraintCount; ++i)
        {
            setStage("transformConstraints[" + std::to_string(i) + "]");
            reader.String();
            const int transformBoneCount = reader.VarInt(true);
            for (int b = 0; b < transformBoneCount; ++b)
                reader.VarInt(true);
            reader.VarInt(true);
            reader.Float();
            reader.Float();
        ...
        setStage("pathConstraints.count");
        const int pathConstraintCount = reader.VarInt(true);
        if (failIfBad()) return false;
        if (pathConstraintCount < 0 || pathConstraintCount > 100000)
        {
            m_lastError = "Spine 3.4 binary load failed: invalid path constraint count.";
            return false;
        }
        for (int i = 0; i < pathConstraintCount; ++i)
        {
            setStage("pathConstraints[" + std::to_string(i) + "]");
            reader.String();
            const int pathBoneCount = reader.VarInt(true);
            for (int b = 0; b < pathBoneCount; ++b)
                reader.VarInt(true);
            reader.VarInt(true);
            reader.VarInt(true);
            reader.VarInt(true);
        ...
        setStage("animations.count");
        const int animationCount = reader.VarInt(true);
        if (failIfBad()) return false;
        if (animationCount < 0 || animationCount > 100000)
        {
            m_lastError = "Spine 3.4 binary load failed: invalid animation count.";
            return false;
        }
        for (int i = 0; i < animationCount; ++i)
        {
            setStage("animations[" + std::to_string(i) + "].name");
            const std::string animationName = reader.String();
            AnimationData animation{};
            animation.name = animationName;
            setStage("animations[" + std::to_string(i) + "]:" + animationName);
        ...
        ForEachObjectMember(jsonText, skinsBegin, skinsEnd, [this, &jsonText](const std::string& skinName, std::size_t skinBegin, std::size_t skinEnd)
        {
            ForEachObjectMember(jsonText, skinBegin, skinEnd, [this, &jsonText, &skinName](const std::string& slotName, std::size_t slotBegin, std::size_t slotEnd)
            {
                ForEachObjectMember(jsonText, slotBegin, slotEnd, [this, &jsonText, &skinName, &slotName](const std::string& attachmentName, std::size_t attachmentBegin, std::size_t attachmentEnd)
                {
                    ...
                    if (!typeValue.empty() && typeValue != "region" && typeValue != "mesh" &&
                        typeValue != "skinnedmesh" && typeValue != "linkedmesh")
                        return;
                    attachment.mesh = typeValue == "mesh" || typeValue == "linkedmesh";
                    attachment.skinnedMesh = typeValue == "skinnedmesh";
                    attachment.linkedMesh = typeValue == "linkedmesh";
                    ...
                    ReadStringPropertyInObject(jsonText, attachmentBegin, attachmentEnd, "skin", attachment.parentSkinName);
                    ReadStringPropertyInObject(jsonText, attachmentBegin, attachmentEnd, "parent", attachment.parentAttachmentName);
                    ...
                    if (attachment.mesh || attachment.skinnedMesh)
                    {
                        ...
                        const bool weighted = attachment.skinnedMesh || rawVertices.size() != attachment.uvs.size();
                        if (weighted)
                        {
    ...
    {
        reader.SetContext("skin " + skinName + " slotGroupCount");
        const int slotAttachmentGroups = reader.VarInt(true);
        if (!reader.Ok() || slotAttachmentGroups < 0 || slotAttachmentGroups > 100000)
            return;
        for (int i = 0; i < slotAttachmentGroups; ++i)
        {
            reader.SetContext("skin " + skinName + " slotGroup[" + std::to_string(i) + "]");
            const int slotIndex = reader.VarInt(true);
            const int attachmentCount = reader.VarInt(true);
            if (!reader.Ok() || attachmentCount < 0 || attachmentCount > 100000)
                return;
            for (int j = 0; j < attachmentCount; ++j)
            {
                reader.SetContext("skin " + skinName + " slotGroup[" + std::to_string(i) + "] attachment[" + std::to_string(j) + "]");
    ...
    bool ReadBinaryVertices34(BinaryReader& reader, int vertexCount, RegionAttachment& attachment)
    {
        if (vertexCount <= 0)
            return false;
        ...
        if (!reader.Bool())
        {
            attachment.vertices = reader.FloatValues(vertexCount * 2, 1.0f);
            attachment.skinnedMesh = false;
            return !attachment.vertices.empty();
        }

        attachment.skinnedMesh = true;
        attachment.weightedVertices.clear();
        attachment.weightedVertices.reserve(static_cast<std::size_t>(vertexCount));
        for (int vertex = 0; vertex < vertexCount; ++vertex)
        {
            ...
            {
                RegionAttachment::WeightedBone weight{};
                weight.bone = reader.VarInt(true);
                weight.x = reader.Float();
                weight.y = reader.Float();
                weight.weight = reader.Float();
                weights.push_back(weight);
            }
            attachment.weightedVertices.push_back(std::move(weights));
        }
        return !attachment.weightedVertices.empty();
    }
                              ...
                              RegionAttachment& attachment)
    {
        ...
        case 1:
        {
            RegionAttachment ignored{};
            const int vertexCount = reader.VarInt(true);
            ReadBinaryVertices34(reader, vertexCount, ignored);
            if (nonessential)
                reader.Int32();
            return false;
        }
        case 2:
        {
            ...
            attachment.color = DecodePackedColor(reader.Int32());
            const int vertexCount = reader.VarInt(true);
            attachment.uvs = reader.FloatValues(vertexCount * 2, 1.0f);
            attachment.indices = reader.ShortArray();
            ReadBinaryVertices34(reader, vertexCount, attachment);
            reader.VarInt(true);
            if (nonessential)
            {
                reader.ShortArray();
            ...
            return !attachment.uvs.empty() && !attachment.indices.empty() &&
                (!attachment.vertices.empty() || !attachment.weightedVertices.empty());
        }
        case 3:
        {
            attachment.mesh = true;
            attachment.linkedMesh = true;
            attachment.path = reader.String();
            if (attachment.path.empty())
                attachment.path = name;
            attachment.color = DecodePackedColor(reader.Int32());
            attachment.parentSkinName = reader.String();
            attachment.parentAttachmentName = reader.String();
            attachment.inheritDeform = reader.Bool();
            if (nonessential)
            {
                attachment.width = reader.Float();
                attachment.height = reader.Float();
        ...
        case 4:
        {
            ...
            const int vertexCount = reader.VarInt(true);
            ReadBinaryVertices34(reader, vertexCount, ignored);
            const int lengthsCount = vertexCount / 3;
        ...
        default:
            reader.SetContext("unsupported attachment type " + std::to_string(type) + " in " + defaultName);
            return false;
        }
    }

    void ResolveLinkedMeshes()
    {
        std::vector<std::pair<std::string, std::string>> links;
        for (const auto& entry : m_attachments)
        {
            if (entry.second.linkedMesh)
                links.push_back({ entry.first, entry.second.parentAttachmentName });
        }

        for (const auto& link : links)
        {
            auto linkedIt = m_attachments.find(link.first);
            if (linkedIt == m_attachments.end())
                continue;

            RegionAttachment& linked = linkedIt->second;
            if (!linked.vertices.empty() || !linked.weightedVertices.empty())
                continue;

            std::string ownSkin;
            std::string slotName;
            SplitSkinAttachmentKey(link.first, ownSkin, slotName);

            const std::string parentName = linked.parentAttachmentName.empty() ? linked.name : linked.parentAttachmentName;
            const RegionAttachment* parent = nullptr;
            if (!linked.parentSkinName.empty())
                parent = FindAttachmentInSkin(linked.parentSkinName, slotName, parentName);
            if (parent == nullptr)
                parent = FindAttachmentInSkin(ownSkin, slotName, parentName);
            if (parent == nullptr && ownSkin != "default")
                parent = FindAttachmentInSkin("default", slotName, parentName);
            if (parent == nullptr)
                continue;

            const std::string ownPath = linked.path;
            const Color ownColor = linked.color;
            const float ownWidth = linked.width;
            const float ownHeight = linked.height;
            const bool ownInheritDeform = linked.inheritDeform;

            linked.mesh = parent->mesh;
            linked.skinnedMesh = parent->skinnedMesh;
            linked.vertices = parent->vertices;
            linked.weightedVertices = parent->weightedVertices;
            linked.uvs = parent->uvs;
            linked.indices = parent->indices;
            linked.path = ownPath.empty() ? parent->path : ownPath;
            linked.color = ownColor;
            linked.width = ownWidth > 0.0f ? ownWidth : parent->width;
            linked.height = ownHeight > 0.0f ? ownHeight : parent->height;
            linked.inheritDeform = ownInheritDeform;
        }
    }

    const RegionAttachment* FindAttachmentInSkin(const std::string& skinName, const std::string& slotName,
                                                 const std::string& attachmentName) const
    {
        const auto it = m_attachments.find(SkinAttachmentKey(skinName, slotName, attachmentName));
        return it == m_attachments.end() ? nullptr : &it->second;
    }

    static void SplitSkinAttachmentKey(const std::string& key, std::string& skinName, std::string& slotName)
    {
        ...
        const std::size_t second = key.find('\n', first + 1);
        skinName = key.substr(0, first);
        slotName = second == std::string::npos ? std::string() : key.substr(first + 1, second - first - 1);
    }
    ...
    void ParseDeformTimelines(const std::string& jsonText, std::size_t animBegin, std::size_t animEnd,
                              AnimationData& animation)
    {
        std::size_t deformPos = 0;
        if ((!FindObjectMemberValue(jsonText, animBegin, animEnd, "deform", deformPos)) &&
            (!FindObjectMemberValue(jsonText, animBegin, animEnd, "ffd", deformPos)))
        {
            return;
        }
        if (deformPos == 0 || jsonText[deformPos] != '{')
            return;

        std::size_t deformEnd = 0;
        if (!FindMatchingBracket(jsonText, deformPos, '{', '}', deformEnd))
            return;

        ForEachObjectMember(jsonText, deformPos, deformEnd, [this, &jsonText, &animation](const std::string& skinName, std::size_t skinBegin, std::size_t skinEnd)
        {
            ForEachObjectMember(jsonText, skinBegin, skinEnd, [this, &jsonText, &animation, &skinName](const std::string& slotName, std::size_t slotBegin, std::size_t slotEnd)
            {
                const int slotIndex = FindSlotIndex(slotName);
                if (slotIndex < 0)
                    return;

                ForEachObjectMember(jsonText, slotBegin, slotEnd, [this, &jsonText, &animation, &skinName, &slotName, slotIndex](const std::string& attachmentName, std::size_t framesBegin, std::size_t framesEnd)
                {
                    if (framesBegin >= jsonText.size() || jsonText[framesBegin] != '[')
                        return;

                    const RegionAttachment* attachment = FindAttachmentInSkin(skinName, slotName, attachmentName);
                    if (attachment == nullptr)
                        attachment = FindAttachmentInSkin("default", slotName, attachmentName);
                    if (attachment == nullptr)
                        return;

                    std::size_t deformLength = attachment->vertices.size();
                    if (attachment->skinnedMesh)
                    {
                        deformLength = 0;
                        for (const auto& weightedVertex : attachment->weightedVertices)
                            deformLength += weightedVertex.size() * 2;
                    }
                    if (deformLength == 0)
                        return;
                    ...
                    ForEachObjectInArray(jsonText, framesBegin, framesEnd, [this, &jsonText, attachment, deformLength, &timeline, &animation](std::size_t frameBegin, std::size_t frameEnd)
                    {
                        FfdFrame frame{};
                        ReadFloatPropertyInObject(jsonText, frameBegin, frameEnd, "time", frame.time);
                        animation.duration = std::max(animation.duration, frame.time);

                        std::vector<float> values;
                        if (ReadFloatArrayPropertyInObject(jsonText, frameBegin, frameEnd, "vertices", values))
                        {
                            frame.vertices.assign(deformLength, 0.0f);
                            float offsetValue = 0.0f;
                            ReadFloatPropertyInObject(jsonText, frameBegin, frameEnd, "offset", offsetValue);
                            const std::size_t offset = static_cast<std::size_t>(std::max(0.0f, offsetValue));
                            for (std::size_t i = 0; i < values.size() && offset + i < frame.vertices.size(); ++i)
                                frame.vertices[offset + i] = values[i];
                            if (!attachment->skinnedMesh)
                            {
                                const std::size_t count = std::min(frame.vertices.size(), attachment->vertices.size());
                                for (std::size_t i = 0; i < count; ++i)
                                    frame.vertices[i] += attachment->vertices[i];
                            }
                        }
                        else if (!attachment->skinnedMesh)
                        {
                        ...
                        ReadCurve(jsonText, frameBegin, frameEnd, frame);
    ...
    void ReadBinaryBoneTimelines(BinaryReader& reader, AnimationData& animation)
    {
        ...
        for (int i = 0; i < boneTimelineCount; ++i)
        {
            ...
            for (int t = 0; t < timelineCount; ++t)
            {
                ...
                {
                    std::vector<FloatFrame>& frames = type == 1 ? timeline.translate : (type == 2 ? timeline.scale : timeline.shear);
                    for (int f = 0; f < frameCount; ++f)
                    {
                        FloatFrame frame{};
                        frame.time = reader.Float();
    ...
    void SkipBinaryTransformTimelines(BinaryReader& reader, AnimationData& animation)
    {
        const int transformTimelineCount = reader.VarInt(true);
        for (int i = 0; i < transformTimelineCount; ++i)
        {
            reader.VarInt(true);
            const int frameCount = reader.VarInt(true);
            for (int f = 0; f < frameCount; ++f)
    ...
    void SkipBinaryPathTimelines(BinaryReader& reader, AnimationData& animation)
    {
        const int pathTimelineCount = reader.VarInt(true);
        for (int i = 0; i < pathTimelineCount; ++i)
        {
            reader.VarInt(true);
            const int timelineCount = reader.VarInt(true);
            for (int t = 0; t < timelineCount; ++t)
    ...
    {
        reader.SetContext("animation " + animation.name + " slotTimelines");
        ReadBinarySlotTimelines(reader, animation);
        reader.SetContext("animation " + animation.name + " boneTimelines");
        ReadBinaryBoneTimelines(reader, animation);
        reader.SetContext("animation " + animation.name + " ikTimelines");
        ReadBinaryIkTimelines(reader, animation);
        reader.SetContext("animation " + animation.name + " transformTimelines");
        SkipBinaryTransformTimelines(reader, animation);
        reader.SetContext("animation " + animation.name + " pathTimelines");
        SkipBinaryPathTimelines(reader, animation);
        reader.SetContext("animation " + animation.name + " deformTimelines");
        ReadBinaryFfdTimelines(reader, animation);
        reader.SetContext("animation " + animation.name + " drawOrder");
        ReadBinaryDrawOrderTimeline(reader, animation);
        if (reader.Offset() < reader.Size())
        {
            reader.SetContext("animation " + animation.name + " events");
                           ...
                           std::size_t boneAnimEnd, AnimationData& animation)
    {
        ...
        if (FindObjectMemberValue(jsonText, boneAnimBegin, boneAnimEnd, "shear", timelinePos) && jsonText[timelinePos] == '[')
        {
            std::size_t timelineEnd = 0;
            if (FindMatchingBracket(jsonText, timelinePos, '[', ']', timelineEnd))
                ParseFloatTimeline(jsonText, timelinePos, timelineEnd, "x", "y", 0.0f, 0.0f, timeline.shear, animation.duration);
        }

        if (!timeline.rotate.empty() || !timeline.translate.empty() || !timeline.scale.empty() || !timeline.shear.empty())
    ...
    void LoadAnimations(const std::string& jsonText)
    {
        ...
        ForEachObjectMember(jsonText, animationsBegin, animationsEnd, [this, &jsonText](const std::string& animationName, std::size_t animBegin, std::size_t animEnd)
        {
            ...
            ParseDeformTimelines(jsonText, animBegin, animEnd, animation);
...
{
    return std::make_unique<Spine34CppRuntime>();
}
