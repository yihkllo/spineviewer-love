...
namespace custom_titlebar
{
	...
	static HandleToSrvFn s_handleToSrv = nullptr;

	float GetScale() { return s_scale; }
	void  SetScale(float scale) { s_scale = scale < 0.5f ? 0.5f : scale; }

	void SetHandleToSrvProvider(HandleToSrvFn fn)
	{
		s_handleToSrv = fn;
	}

	static ImTextureID GetImTexId(int textureHandle)
	{
		if (textureHandle == -1) return (ImTextureID)0;
		void* srv = s_handleToSrv ? s_handleToSrv(textureHandle) : nullptr;
	...
	void Draw(const STitleBarConfig& cfg)
	{
		...
		if (ImTextureID bgTex = GetImTexId(cfg.hBgTexture))
		{
			dl->AddImage(bgTex,
		...
		float curX = 8.f * sc;
		if (ImTextureID iconTex = GetImTexId(cfg.hIconTexture))
		{
