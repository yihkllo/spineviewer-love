...
csmInt32 CubismMotionJson::GetActualMotionCurveCount() const
{
    csmVector<Utils::Value*>* curves = _json->GetRoot()[Curves].GetVector();
    return curves == NULL ? 0 : static_cast<csmInt32>(curves->GetSize());
}
...
csmInt32 CubismMotionJson::GetActualEventCount() const
{
    csmVector<Utils::Value*>* events = _json->GetRoot()[UserData].GetVector();
    return events == NULL ? 0 : static_cast<csmInt32>(events->GetSize());
}
