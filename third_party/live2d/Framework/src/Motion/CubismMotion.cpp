...
void CubismMotion::Parse(const csmByte* motionJson, const csmSizeInt size)
{
    ...
    _motionData->CurveCount = json->GetActualMotionCurveCount();
    _motionData->Fps = json->GetMotionFps();
    _motionData->EventCount = json->GetActualEventCount();
    ...
    csmInt32 requiredSegmentCount = 0;
    csmInt32 requiredPointCount = 0;
    for (csmInt32 curveIndex = 0; curveIndex < _motionData->CurveCount; ++curveIndex)
    {
        const csmInt32 valueCount = json->GetMotionCurveSegmentCount(curveIndex);
        if (valueCount < 2)
        {
            continue;
        }
        ...
        ++requiredPointCount;
        for (csmInt32 position = 2; position < valueCount;)
        {
            const csmInt32 type = static_cast<csmInt32>(json->GetMotionCurveSegment(curveIndex, position));
            ++requiredSegmentCount;
            if (type == CubismMotionSegmentType_Bezier)
            {
    ...
    _motionData->Segments.UpdateSize(requiredSegmentCount, CubismMotionSegment(), true);
    _motionData->Points.UpdateSize(requiredPointCount, CubismMotionPoint(), true);
