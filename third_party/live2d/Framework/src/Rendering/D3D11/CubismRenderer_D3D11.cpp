...
namespace
{
    CubismRenderState_D3D11* s_renderStateManager = NULL;
    CubismShader_D3D11* s_shaderManagerInstance = NULL;

    csmUint32 s_bufferSetNum = 0;
    ID3D11Device* s_device = NULL;
    ...
    for (csmUint32 buffer = 0; buffer < s_bufferSetNum; buffer++)
    {
        ...
        for (csmUint32 drawAssign = 0; drawAssign < drawableCount; drawAssign++)
        {
            ...
            if (vcount != 0)
            {
                ...
                bufferDesc.ByteWidth = sizeof(CubismVertexD3D11) * vcount;
            ...
            if (icount != 0)
            {
                D3D11_BUFFER_DESC bufferDesc;
                memset(&bufferDesc, 0, sizeof(bufferDesc));
                bufferDesc.ByteWidth = sizeof(WORD) * icount;
