...
namespace
{
	...
	int CompareExplorerNames(const std::wstring& left, const std::wstring& right)
	{
		const int byName = ::StrCmpLogicalW(left.c_str(), right.c_str());
		return byName != 0 ? byName : ::CompareStringOrdinal(left.c_str(), -1, right.c_str(), -1, TRUE) - CSTR_EQUAL;
	}
	...
	void SortByDisplayName(std::vector<std::wstring>& paths, size_t beginIndex)
	{
		...
		std::sort(paths.begin() + static_cast<std::ptrdiff_t>(beginIndex), paths.end(),
			[](const std::wstring& left, const std::wstring& right) {
				const int byFileName = CompareExplorerNames(FileNameOf(left), FileNameOf(right));
				return byFileName != 0 ? byFileName < 0 : CompareExplorerNames(left, right) < 0;
			});
	}
...
void path_util::ScanLive2DModelsRecursive(const std::wstring& folder, std::vector<std::wstring>& outPaths)
{
	const fs::path root(folder);
	std::error_code ec;
	if (!fs::is_directory(root, ec))
	...
	for (; it != end; NextEntry(it, end, ec))
	{
		...
		if (name.size() >= std::wcslen(suffix) &&
			_wcsicmp(name.c_str() + name.size() - std::wcslen(suffix), suffix) == 0)
		{
			outPaths.push_back(entry.path().wstring());
		}
	}
